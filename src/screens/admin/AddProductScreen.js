...
const AddProductScreen = ({ navigation }) => {
  ...
  const fetchCategories = async () => {
    try {
      ...
      SweetAlert.error('Error', 'Failed to fetch categories');
    }
  };
  ...
  const pickImage = async () => {
    try {
      ...
      if (permissionResult.granted === false) {
        SweetAlert.warning('Permission Required', 'Permission to access camera roll is required!');
        return;
      }
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        SweetAlert.success('Success', 'Image selected successfully!');
      } else {
        console.log('Image selection was canceled');
      }
    } catch (error) {
      console.error('Error picking image:', error);
      SweetAlert.error('Error', 'Failed to pick image. Please try again.');
    }
  };

  const handleAddProduct = async () => {
    if (!name || !description || !price || !stock || !categoryId) {
      SweetAlert.warning('Missing Information', 'Please fill in all required fields including category');
      return;
    }
    ...
    if (isNaN(priceNum) || priceNum <= 0) {
      SweetAlert.error('Invalid Price', 'Please enter a valid price');
      return;
    }

    if (isNaN(stockNum) || stockNum < 0) {
      SweetAlert.error('Invalid Stock', 'Please enter a valid stock quantity');
      return;
    }
    ...
    try {
      ...
      SweetAlert.success('Success', 'Product added successfully!', () => navigation.goBack());
    } catch (error) {
      console.error('Error adding product:', error);
      SweetAlert.error('Error', `Failed to add product: ${error.message}`);
    } finally {
      setLoading(false);
    }
  };
